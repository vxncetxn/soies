...
| File | Role |
|------|------|
| [`package.json`](../package.json) | Dependencies, scripts (`start`, `ios`, `android`, `lint`, `lint:rc`, `healthcheck:rc`, `check`, `typecheck`, `fmt`). Entry point: **`expo-router/entry`** (canonical; custom `index.js` and `.pnpm` `watchFolders` were removed after the Metro matrix proved them unnecessary). `expo.autolinking.ios.buildFromSource` forces Reanimated + Worklets to compile from source on iOS. `pnpm.patchedDependencies` pins Worklets' temporary Metro 0.84.4 generated-module SHA-1 fix and Metro Runtime 0.84.4 Fast Refresh forwarding fix; re-evaluate and regenerate or remove both patches whenever Metro or Worklets changes. |
...
| [`metro.config.js`](../metro.config.js) | Metro + Uniwind (`cssEntryFile`, `dtsFile`), followed by Worklets' `getBundleModeMetroConfig` wrapper so it preserves all earlier config modifications. No `unstable_enableSymlinks` (Metro 0.84 always-on; Expo Doctor rejects the override). No extra `watchFolders`. |
