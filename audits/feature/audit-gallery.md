...
**Follow-up reviewer re-assessment (2026-07-15):** G-02, G-05, G-08, and G-12 can now be closed as Fully addressed. The code defects under G-01, G-15, and G-09 are fixed, but those findings still require their explicit physical-device performance, visual, and screen-reader acceptance checks before comprehensive closure. Prior follow-ups under G-03, G-11, and G-14 are also resolved, subject to the missing-CI documentation caveat recorded below.
...
| ---- | -------- | ------------------------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| G-14 | High     | The required 10-artefact Gallery limit is not enforced                                            | Yes       | Added a typed `GalleryCapacityError`, transactional count, and migration V3 insert/revival triggers covering direct sync/import writes. Hidden parent memberships count. Tests cover artefacts 10/11, removal, revival, duplicate-at-capacity, and racing writes; `pnpm check` now runs the suite as part of the release gate.                                                                                                                      | **Fully addressed.** The transactional guard and schema triggers remain authoritative, and `pnpm check` now executes all 11 regression tests, closing the prior local aggregate-check gap. The race test still uses one synchronous in-memory connection rather than two OP-SQLite clients, but the database trigger protects the invariant independently. One documentation caveat remains outside this finding: the repository claims a `.github/workflows/quality.yml` CI gate that does not exist, so `pnpm check` is currently a local release gate rather than an automatically enforced one.                                        |
| G-01 | Medium   | A bounded Gallery still duplicates expensive native content per artefact                          | Yes       | Retained the single pager-owned overlay and added an explicit focus-session state machine. A closing session keeps ownership through native settle, ignores reopen attempts, and keys deferred removal to its own target; the completion ref synchronizes before close animation, including immediate/reduced-motion settle. A regression test covers close/remove/reopen ordering. The physical 10-artefact Print+Ink profile remains outstanding. | **Addressed in code; device sign-off pending.** The focus-session state machine keeps target and deferred-removal ownership through close settle, rejects reopen attempts while closing, and the layout-effect callback synchronization covers immediate settle. The focused test verifies that only the owning artefact can be removed; no remaining wrong-target path was found. The original allocation issue and the High-severity follow-up race are therefore fixed, but the finding cannot yet be closed as **Fully addressed** until the explicitly required physical 10-item Print+Ink launch, memory, and scroll profile passes. |
| G-15 | Medium   | The frame preserves portrait geometry but only partially ports the reference surface treatment    | Yes       | Resized the mat-centre highlight so it extends beyond the opaque well and remains visible in the mat ring. The deliberate `contain` treatment, reduced depth stack, and inner edge remain unchanged. Reference-screenshot approval on representative iOS/Android devices remains outstanding.                                                                                                                                                       | **Addressed in code; visual and performance sign-off pending.** The highlight is now approximately 1.267 times the well width and 1.241 times its height, so it extends into the mat ring and is no longer fully occluded by the well. The dead-view defect is fixed without adding a rendering dependency, and the other deliberate mobile adaptations remain reasonable. The finding cannot yet be closed as **Fully addressed** until representative iOS/Android reference screenshots and the full-Gallery device profile are approved.                                                                                                |
...
| G-05 | High     | Rotation/resizing breaks page identity and can add a different artefact from the one displayed    | Yes       | Gallery now settles identity on drag end (using native target offset when available) as well as momentum end. An order signature re-resolves the active artefact on same-width refresh/reorder; deletion retains its separate animated clamp. Focused tests cover drag settlement and reorder identity restoration.                                                                                                                                 | **Fully addressed.** Gallery identity now settles on drag end as well as momentum end, using `targetContentOffset` when available. Width or order changes re-resolve the active artefact ID, while the deletion path retains its intentional animated clamp. The extracted production helpers are covered for drag settlement and same-width reorder, and no conflicting path was found.                                                                                                                                                                                                                                                   |
...
| G-08 | Medium   | Artefact delete/restore no longer restores Gallery membership                                     | Yes       | The integration test now invokes the public `softDeleteArtefact()` and `restoreArtefact()` repository operations, verifying that Gallery membership remains active, disappears through the live-parent join, and reappears after Undo.                                                                                                                                                                                                              | **Fully addressed.** The regression test now exercises the public `softDeleteArtefact()` and `restoreArtefact()` repository operations and verifies hidden-on-delete, membership preservation, and visible-on-Undo behavior. It would now catch the repository regression identified in the previous review. The supporting test loader should narrow its broad resolver catch to `ERR_MODULE_NOT_FOUND` for clearer diagnostics, but that low-severity hygiene improvement does not weaken this closure.                                                                                                                                  |
| G-09 | Medium   | Closed Gallery overlays remain exposed to assistive technology                                    | Yes       | Gallery now has one transient overlay. `FocusOverlay` hides closed descendants from VoiceOver/TalkBack and applies modal semantics while open. Manual traversal on both screen readers remains a release check.                                                                                                                                                                                                                                     | **Addressed in code; accessibility sign-off pending.** The one transient overlay is excluded from the accessibility tree while closed and is modal while open, and the G-01 close/reopen race no longer undermines those states. No remaining static accessibility-tree defect was found. The finding cannot yet be closed as **Fully addressed** until VoiceOver and TalkBack traversal is verified on representative devices as required by the original resolution.                                                                                                                                                                     |
...
| G-11 | Low      | Required formatting fails on two new files                                                        | Yes       | Ran `pnpm fmt`; `pnpm fmt:check` and the aggregate `pnpm check` pass. The aggregate check now also runs `pnpm test`.                                                                                                                                                                                                                                                                                                                                | **Fully addressed.** `pnpm check` passes and now includes formatting, types, lint, React Compiler health, and all 11 tests. This closes the formatting and local aggregate-check gaps. The documented `.github/workflows/quality.yml` is absent, however, so these checks are not yet automatically enforced by the claimed CI workflow.                                                                                                                                                                                                                                                                                                   |
| G-12 | Low      | Dead/scaffolding exports and branches remain in the production change set                         | Yes       | Removed the unused public `closeGalleryAdd` context capability. `FRAME_WELL_ASPECT` and `FRAME_MAT_SCALE` are now module-private; the externally consumed `FRAME_BOARD_SCALE` remains exported.                                                                                                                                                                                                                                                     | **Fully addressed.** The unused `closeGalleryAdd` capability has been removed, `FRAME_WELL_ASPECT` and `FRAME_MAT_SCALE` are module-private, and the externally consumed board-scale export remains. No residual API or constant identified by the previous review remains.                                                                                                                                                                                                                                                                                                                                                                |
