...
test("Paper dismissal prepares native responders before starting overlay close", () => {
  ...
  const handleClose = createPaperSource.match(
    /const handleClose = useCallback\([\s\S]*?(?=const \{ saving, submit \})/,
  );
  assert.ok(handleClose, "CreatePaperScreen must own a discoverable two-phase close handler");
...
test("Paper keyboard toolbar gives its animated sticky layer non-zero native bounds", () => {
  const stickyLayerStyle = toolbarSource.match(/stickyLayer:\s*\{[\s\S]*?\n\s*\},/);

  assert.ok(stickyLayerStyle, "PaperTextPresetToolbar must define its sticky native layer");
  assert.match(toolbarSource, /const TOOLBAR_HEIGHT = \d+/);
